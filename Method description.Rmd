...
Here we test causal relationships of events in the delivery and impact of agro-climate services (ACS) in two farmers groups in Dien Bien, Vietnam. The tests are programmed in the R programming language [@r_core_team_r:_2020]. We use the `gsDesign` package [@anderson_gsdesign_2021] for the specific tests. The full testing procedure is available in the [ACS-adoption-Pathway repository](https://github.com/ThiThuGiangLuu/ACS-adoption-decision-pathway). 

### Variables collected in a farmer survey 

`diffusion-variable-description.xlsx` contains descriptions of variables, variable values and questions that we asked to gather the necessary data for that variable. Download the table from the [ACS-adoption-Pathway repository](https://github.com/ThiThuGiangLuu/ACS-adoption-decision-pathway).

The data table `diffusion.csv` contains the variable names and values. Download the table from the [ACS-adoption-Pathway repository](https://github.com/ThiThuGiangLuu/ACS-adoption-decision-pathway).
...
The testing function `test_relationship` is designed to validate causal relationships obtained from farmer surveys. The tested causal relationships are between causal events and resulting events (both binomially distributed). We perform the test by comparing two **success** rates of the **resulting event** brought about in the presence and absence of the hypothesized **causal event**. The two success rates are calculated as: 

1. the probability of observing **successful event** together with **causal event** [yes] observations and 

2. the probability to observe **successful event**/ [yes] observations together with **causal event** [no] observations.  
...
We test the causal relations of each sub-hypothesis described in the hypothesized ACS delivery and adoption impact pathway using testing function `test_relationship`. For example, with the following code, we test the difference between access rate among village saving and loan association (VSLA) farmers needing and not needing ACS:
...
We test outcomes of the relationship between "need" and "access", in which:
...
The full data set and the R scripts describing the testing procedure is available in the [ACS-adoption-Pathway repository](https://github.com/ThiThuGiangLuu/ACS-adoption-decision-pathway). 
