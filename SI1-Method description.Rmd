---
title: "**Supplementary materials:** <br /> The last mile: Different routes may lead to the same adoption result, thanks to need conciousness, interpersonal relations and positive attitudes
 for agro-climate services in farmer groups"
...
```{r, out.width="90%", fig.align="center", fig.cap="Fig. The hypothesized agro-climate service (ACS) delivery and adoption impact pathway and the dynamics of ACS adoption in Village saving and loan association (VSLA) and conventional farmer group (non-VSLA) in Muong Phang and Pa Khoang communes, Dien Bien District, Vietnam.", echo=FALSE}
knitr::include_graphics('Figures-tables/Fig1. Adoption-Delivery-Adoption-Pathway-Dynamics.jpg', dpi=NA)
```


### 1.2. Farmer survey
`SI2-Questionnaire.pdf` contains the set of questions used in the farmer survey, aiming for testing the conceptual impact pathways. Download the survey tool from the [ACS-adoption-decision-pathway](https://github.com/ThiThuGiangLuu/ACS-adoption-decision-pathway)
...
```{r, out.width="70%", fig.align="center", fig.cap="Table. Quantification of the strength of relations between binomially distributed (hypothesized) causal and resulting events. The quantification is based on the interpretation of confidence interval (CI) for the difference of the probabilities p1 and p2 of the “successful event”/ [yes] attributable to [yes] and [no] observations of the hypothesized causal event. The width of CI is compared with a pre-defined range of practical indifference. Depending on the range of CI, we considered the hypothesized causal and resulting events to have no, potential, weak, moderate or strong relationships", echo=FALSE}
